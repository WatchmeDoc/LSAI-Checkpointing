...
static void mapPersistentRegion(const char *filename, size_t *regionAddr, const uint64_t regionSize, bool data, int fd)
{
    ...
    memset(PR_ADDR, 0, regionSize);
}

//====================================================================

/* AArch64 translation of clwb + sfence  (GCC/Clang) */
static inline void FLUSH(void *p)
{
#if defined(__aarch64__)
# ifdef __ARM_FEATURE_DPB          /* Arm v8.2-A Persistent-mem support */
    asm volatile ("dc  cvap, %0"   :: "r"(p) : "memory");   /* to PoP */
# else                              /* anything pre-v8.2-A */
    asm volatile ("dc  cvac, %0"   :: "r"(p) : "memory");   /* to PoC */
# endif
#else
    /* keep the original clwb on x86/x86-64 */
...
{
#if defined(__aarch64__)
    asm volatile ("dsb ish" ::: "memory");   /* full persist fence */
#else
...
{
    FLUSH(p);      /* push the line out of the D-cache          */
    SFENCE();      /* make the write and the flush globally visible */
}
